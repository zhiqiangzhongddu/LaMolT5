...
parser.add_argument('--input_file', type=str, default='/home/haoyu.guo/Synthetic/predictions/molt5-augmented-default-2-large-smiles2caption.txt', help='path where test generations are saved')

# parser.add_argument('--input_file', type=str, default='/home/haoyu.guo/Synthetic/predictions/molt5-augmented-contrastive-100-small-smiles2caption.txt', help='path where test generations are saved')
...
parser.add_argument('--log_fp', type=str, default='result_text2mol.txt')
# parser.add_argument('--log_fp', type=str, default='result_text2mol_contrastive.txt')
...
print('Average Similarity:', a_sim)

text_embs = np.array(text_embs).squeeze()
mol_embs = np.array(mol_embs).squeeze()

mat = cosine_similarity(text_embs, mol_embs)
n_sim = np.mean(mat[np.eye(mat.shape[0]) == 0])
print('Negative Similarity:', n_sim)


log_io = open(args.log_fp, 'a+')

log_io.write(f'For {args.input_file}\n')
log_io.write(f'Average Similarity: {a_sim}\n')
log_io.write(f'Negative Similarity: {n_sim}\n')
log_io.write('\n')
log_io.flush()
log_io.close()